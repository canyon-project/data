...
fn merge_file_coverage(first: &Value, second: &Value) -> Value {
...
// Expose the merge function to JavaScript
#[wasm_bindgen]
pub fn merge_coverage_json_str(first_json: &str, second_json: &str) -> String {
    let first_value: Value = serde_json::from_str(first_json).unwrap();
    let second_value: Value = serde_json::from_str(second_json).unwrap();
    let merged_value = merge_coverage_map(&first_value, &second_value);
    serde_json::to_string(&merged_value).unwrap()
}